...
test("the same agent uses its stable handle for messages, sidebar and live Activity", () => {
  const agent = { id: "agent-1", name: "dev-bot-2", displayName: "Dev Bot 2" };

  assert.equal(avatarSeedFor(agent, "dev-bot-2"), "dev-bot-2");
  assert.equal(avatarSeedFor({ ...agent, displayName: "New display name" }, "Dev Bot 2"), "dev-bot-2");
});
